// ==UserScript==
// @name         ZonaTMO - Exportar Pendientes (con imágenes)
// @namespace    http://tampermonkey.net/
// @version      2.2
// @description  Exporta listas de ZonaTMO a TXT, JSON, HTML visual y Excel
// @author       You
...
(function () {
    ...
    function exportarTitulos() {
        const titulos = [];
        document.querySelectorAll('#list-elements .element').forEach(el => {
...
<div class="grid">${cards}</div>
    ...
    function mostrarModal(datos, nombre) {
        ...
        modal.querySelector('#tmo-close').onclick = () => modal.remove();
        ...
        modal.querySelector('#tmo-html').onclick = () => {
            setProgress(`✅ Generando HTML con ${datos.length} portadas...`);
            setTimeout(() => {
    ...
    function crearBoton() {
        ...
        btn.textContent = '📥 Exportar Lista';
        ...
        btn.addEventListener('click', () => {
            ...
            if (datos.length === 0) { alert('Primero selecciona una lista en el dropdown.'); return; }
            mostrarModal(datos, getNombreLista());
        });
    ...
    function iniciar() {
        ...
        if (!lista) { setTimeout(iniciar, 500); return; }
        crearBoton();
